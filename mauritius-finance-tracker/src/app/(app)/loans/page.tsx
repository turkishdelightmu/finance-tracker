    ...
    <div className="space-y-6 pb-20">
      ...
      <section className="rounded-3xl bg-white/90 border border-slate-200 p-5">
        ...
        <form action={createLoanAction} className="mt-4 grid gap-3 md:grid-cols-2">
          <input name="name" placeholder="Loan name" className="rounded-xl border border-slate-200 px-3 py-2" required />
          <input name="principal" placeholder="Principal" className="rounded-xl border border-slate-200 px-3 py-2" required />
          <input name="apr" placeholder="APR (%)" className="rounded-xl border border-slate-200 px-3 py-2" required />
          <input name="termMonths" placeholder="Term (months)" className="rounded-xl border border-slate-200 px-3 py-2" required />
          <input name="paymentDay" placeholder="Payment day (1-28)" className="rounded-xl border border-slate-200 px-3 py-2" required />
          <input name="monthlyPayment" placeholder="Monthly payment" className="rounded-xl border border-slate-200 px-3 py-2" required />
          <input name="currentBalance" placeholder="Current balance" className="rounded-xl border border-slate-200 px-3 py-2" required />
      ...
      <section className="rounded-3xl bg-white/90 border border-slate-200 p-5">
        ...
        <div className="mt-4 space-y-3">
          ...
          {loans.map((loan: LoanRow) => {
            ...
            return (
              <div key={loan.id} className="border border-slate-200 rounded-2xl p-4">
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-2">
                  ...
                  <form action={overrideBalanceAction} className="flex gap-2">
                    <input type="hidden" name="loanId" value={loan.id} />
                    <input name="balance" placeholder="Override balance" className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm" />
